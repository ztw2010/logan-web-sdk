...
export default class LoganDB {
  ...
  async getLogDayInfo(logDay: string): Promise<LoganLogDayItem | null> {
    return (await this.DB.getItem(LOG_DAY_TABLE_NAME, logDay)) as any as LoganLogDayItem | null
  }
  async getLogDaysInfo(fromLogDay: string, toLogDay: string): Promise<LoganLogDayItem[]> {
    if (fromLogDay === toLogDay) {
      const result = (await this.DB.getItem(LOG_DAY_TABLE_NAME, fromLogDay)) as any as LoganLogDayItem | null
  ...
  async getLogsByReportName(reportName: FormattedLogReportName): Promise<LoganLogItem[]> {
  ...
  async addLog(logString: string): Promise<void> {
    ...
    const needNewPage = currentPageSize > 0 && currentPageSize + logSize > DEFAULT_SINGLE_PAGE_MAX_SIZE
