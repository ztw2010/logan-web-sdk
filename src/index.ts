import { LogEncryptMode, ReportConfig, GlobalConfig, LogConfig } from './interface'
...
async function logAsync(logItem: LogConfig): Promise<void> {
  ...
  if (LogManager.canSave()) {
    try {
      await saveLog(logItem)
    } catch (e) {
      LogManager.errorTrigger()
      await Config.get('errorHandler')(e)
    }
  } else {
    await Config.get('errorHandler')(new Error(ResultMsg.EXCEED_TRY_TIMES))
  }
}
